...
*/
    ...
    function findSmallestInt(args){
      ...
      let currentLowest = args[0]; // set the current lowest to the first element in the array
      for(let i = 1; i < args.length;i++){
        
        //itterate through the array checking each if it is lower than the current lowest element
            if(args[i] < currentLowest){
                currentLowest = args[i]
        //if the current lowest is higher than a integer at the given position, the current lowest is change to that one. 
            }
      }
